...
export const ClientesTable: React.FC = () => {
  ...
  const handleSelectAllChange = (checked: boolean) => {
    if (checked) {
      setSelectedUsers(new Set(users.filter(user => user.category === "Cliente").map((user) => user.email)));
    } else {
  ...
  const filteredUsers = users.filter((user) => {
    ...
    return user.category === "Cliente" && matchesSearchQuery && matchesStatusFilter;
  });
